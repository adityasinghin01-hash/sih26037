...
%
%   The +sc scenario builders (s1actors.m, s2actors.m, in the OTHER repo,
%   ~/Desktop/SIH26037-Reference/matlab) key their `who` map by a short role
%   tag, not by the S5 class name (sih.util.classNames) - and S1 and S2 do not
%   even agree with each other: S1 uses 'moto_wrong'/'moto_over' for its two
%   motorbike roles, S2 uses plain 'wrong' for the same role, and S2 alone
%   adds 'bus' and 'ace'. This is the one place every tag either scenario has
%   ever used is mapped, so sih.scenario/+perception and +sc can never
%   silently disagree about what a tag means.
%
...
MAP = containers.Map( ...
    {'cow', 'auto', 'moto_wrong', 'moto_over', 'wrong', 'tractor', 'trolley', 'bus', 'ace'}, ...
    {10,     4,       5,            5,           5,       14,        13,       3,     7});
