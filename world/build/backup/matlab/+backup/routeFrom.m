...
if ~isempty(opts.classes)
    ...
    assert(any(keep), "backup:noSuchClass", "no pieces of class %s", strjoin(opts.classes,", "));
    idxMap = find(keep);
    R = R(keep);
else
    idxMap = 1:numel(R);
...
while true
    ...
    if isempty(path_m)
        ...
        assert(gap <= opts.joinTol_m, "backup:openJoin", ...
...
assert(size(path_m,1) >= 2, "backup:emptyRoute", "route chained no pieces");

% ---- assertions: the route must be monotone-ish and closed ----
d = vecnorm(diff(path_m),2,2);
assert(all(d > 0), "backup:dupPoint", "route contains a zero-length step");
